...
:mSystem(parent)
{
   ...
   extern int lynx_audio_sample_rate;
   sblip = blip_new(lynx_audio_sample_rate / 10);
   blip_set_rates(sblip, HANDY_SYSTEM_FREQ, lynx_audio_sample_rate);
...
inline void CMikie::UpdateSound(void)
{
   ...
   sample_l = ((sample_l << 4) * master_volume) / 100;
   sample_r = ((sample_r << 4) * master_volume) / 100;
